    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-20">
      ...
      {activeSubTab === 'general' && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
          ...
          <section className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
             <div className="flex justify-between items-start mb-8"><h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight flex items-center gap-3"><ShieldCheck className="text-primary" /> Backup em Nuvem</h3><div className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${data.settings.googleSyncEnabled ? 'bg-green-50 text-green-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>{data.settings.googleSyncEnabled ? 'Conectado' : 'Offline'}</div></div>
             {data.settings.googleSyncEnabled ? (
               <div className="grid sm:grid-cols-2 gap-4"><button onClick={handleManualSync} disabled={isSyncing} className="flex items-center justify-center gap-3 p-5 bg-primary text-white rounded-3xl font-black uppercase text-[10px] tracking-widest hover:brightness-110 shadow-xl transition-all"><RefreshCw size={18} className={isSyncing ? 'animate-spin' : ''} /> Sincronizar Agora</button><button onClick={() => onUpdateData({ settings: { ...data.settings, googleSyncEnabled: false }})} className="p-5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-3xl font-black uppercase text-[10px] tracking-widest hover:text-red-500 transition-all"><LogOut size={18} className="inline mr-2" /> Desconectar</button></div>
             ) : (
               <button onClick={() => onUpdateData({ settings: { ...data.settings, googleSyncEnabled: true }})} className="w-full flex items-center justify-center gap-4 py-6 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-[32px] font-black uppercase text-xs tracking-widest hover:border-primary transition-all dark:text-white">Conectar com Google Drive</button>
             )}
          </section>
